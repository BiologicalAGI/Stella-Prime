...
function Add-ReadError([string]$Label, [string]$Path, [string]$Message) {
    Write-Host ("READ_ERROR|LABEL={0}|MESSAGE={1}|PATH={2}" -f $Label,$Message,$Path)
...
function Check-ExactFileHash(
    ...
    try {
        ...
        Add-ReadError $Label $Path $_.Exception.Message
    }
}
...
function Check-HashMatchInRoot(
    ...
    try {
        ...
        $Matches = @()
        $Candidates = @(Get-ChildItem -LiteralPath $Root -File -Recurse -ErrorAction Stop |
        ...
        foreach ($File in $Candidates) {
            try {
                ...
                Add-ReadError ($Label + '_CANDIDATE') $File.FullName $_.Exception.Message
            }
        }
        ...
        Add-ReadError $Label $Root $_.Exception.Message
    }
}
...
try {
    ...
    foreach ($Pair in @(
        ...
        try {
            ...
            Add-ReadError $Pair[0] $Pair[1] $_.Exception.Message
        }
    }
    ...
    try {
        ...
        Add-ReadError 'WORLD_KERNEL_ENUMERATION' $WKRoot $_.Exception.Message
    }
    ...
    foreach ($Root in $ExeRoots) {
        try {
            if (Test-Path -LiteralPath $Root -PathType Container -ErrorAction Stop) {
                $Executables = @(Get-ChildItem -LiteralPath $Root -File -Recurse -ErrorAction Stop |
                    Where-Object { $_.Name -in @('luanti.exe','minetest.exe') })
                foreach ($Exe in $Executables) {
                    try {
                        $Hash = (Get-FileHash -LiteralPath $Exe.FullName -Algorithm SHA256 -ErrorAction Stop).Hash
                        ...
                        Add-ReadError 'LUANTI_EXECUTABLE_CANDIDATE' $Exe.FullName $_.Exception.Message
                    }
                }
            }
        }
        catch {
            Add-ReadError 'LUANTI_ROOT_ENUMERATION' $Root $_.Exception.Message
        }
    }
    ...
    try {
        ...
        Add-ReadError 'PROCESS_ENUMERATION' 'Get-Process' $_.Exception.Message
    }

    Section '7. V2-02 ACTIVE CODE / RECEIPTS'
    try {
        if (Test-Path -LiteralPath $V02Root -PathType Container -ErrorAction Stop) {
            $DialogueFiles = @(Get-ChildItem -LiteralPath $V02Root -File -Recurse -Filter 'init.lua' -ErrorAction Stop |
            ...
            $InhabitantFiles = @(Get-ChildItem -LiteralPath $V02Root -File -Recurse -Filter 'init.lua' -ErrorAction Stop |
        ...
        Add-ReadError 'V2_02_ACTIVE_CODE_ENUMERATION' $V02Root $_.Exception.Message
    }
    ...
    try {
        if (Test-Path -LiteralPath $V02Root -PathType Container -ErrorAction Stop) {
            $WorldFiles = @(Get-ChildItem -LiteralPath $V02Root -File -Recurse -Filter 'world.mt' -ErrorAction Stop)
            Write-Host ("WORLD_MT_CANDIDATE_COUNT={0}" -f $WorldFiles.Count)
            $MatchingWorlds = @()
            foreach ($WorldFile in $WorldFiles) {
                try {
                    $Text = Get-Content -LiteralPath $WorldFile.FullName -Raw -ErrorAction Stop
                    ...
                    Add-ReadError 'WORLD_MT' $WorldFile.FullName $_.Exception.Message
                }
            }
        ...
        Add-ReadError 'WORLD_MT_ENUMERATION' $V02Root $_.Exception.Message
    }
    ...
    try {
        if (Test-Path -LiteralPath $RuntimeLog -PathType Leaf -ErrorAction Stop) {
            ...
            try {
                ...
                Add-ReadError 'RUNTIME_LOG_SCAN' $RuntimeLog $_.Exception.Message
            }
        ...
        Add-ReadError 'V2_02R1B_RUNTIME_LOG' $RuntimeLog $_.Exception.Message
    }
    ...
    try {
        if (Test-Path -LiteralPath $V02Root -PathType Container -ErrorAction Stop) {
            $StateDirs = @(Get-ChildItem -LiteralPath $V02Root -Directory -Recurse -Force -ErrorAction Stop |
        ...
        Add-ReadError 'V2_03_TARGET_ENUMERATION' $V02Root $_.Exception.Message
    }
}
catch {
