<#
STELLA V2-03 — MORNING PRE-MUTATION READ-ONLY REVERIFY R1
...
- no use of PowerShell automatic $Matches variable;
- expected size uses a plain Int64 sentinel, not Nullable.Value;
- summary follows try/catch directly, so interactive paste and script-file execution agree.
#>
...
try {
    ...
    Write-Host 'CARD_REVISION=R1_PASTE_SAFE_EXACT_PATH'
    ...
    Check-File 'V0_1_CONFIG_BASELINE' `
        (Join-Path $V01Root 'Luanti\minetest.conf') `
        '3533EEA4FA014548DE059CC7F018E3BD260293FCF0635E79B85979302D91A72E'
    Check-File 'V0_1_CORE_INIT_BASELINE' `
        (Join-Path $V01Root 'Luanti\games\stella_playground\mods\stella_core\init.lua') `
        '65D1140278CF7FC6A149C74C2AD5289628099FFCB8F598DD085B21A1FDD2B716'
    Check-File 'V0_1_RECEIPT_BASELINE' `
        (Join-Path $V01Root 'STELLA_PLAYGROUND_DEPLOYMENT_RECEIPT_V0_1_R1.json') `
        'A80D124CD026ACCF8CA20E3C53F53AB4614CF15E884F0DCF33776DC1466CB346'
    ...
    Check-File 'STELLA_DIALOGUE_INIT' `
        (Join-Path $V02Root 'Luanti\games\stella_v0_2_shell\mods\stella_dialogue\init.lua') `
        '6DC2E9A90CB7772C6C46B627F79D146E1C74397108C2E6AAF22EF37153A754EE'
    Check-File 'STELLA_INHABITANTS_INIT' `
        (Join-Path $V02Root 'Luanti\games\stella_v0_2_shell\mods\stella_inhabitants\init.lua') `
        'FB36E8B3ECE92DA6C73C23BDA88C5C53F6A3A92BA402328735800536B1725519'
    Check-File 'V2_02A_BUILD_RECEIPT' `
        (Join-Path $V02Root 'V2_02A_BUILD_RECEIPT.txt') `
        'B141B05A4FC9A40053CF354FA8FD072707AC1C8041C07D9825D98BD3654A15A0'
    Check-File 'V2_02R1A_R1_REPAIR_RECEIPT' `
        (Join-Path $V02Root 'V2_02R1A_R1_VISUAL_REPAIR_RECEIPT.txt') `
        '76C103F80CE75915B74108D8DCF56BABD9FC79EA4679F019116BE33E58803119'
